<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column label="部署脚本" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.sls | filePathFilter }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="部署脚本">
          <el-select v-model="temp.sls" placeholder="请选择">
            <el-option
...
<script>
import { fetchList, createItem, updateItem, fetchOption, delItem } from '@/api/projects'
...
export default {
  ...
  filters: {
    ...
    filePathFilter(path) {
      return path.replace('/opt/run/flamingos/data/', '')
    },
    toLocaleTime
  },
  data() {
  ...
  methods: {
    getOptions() {
      fetchOption().then(response => {
        ...
        this.FileOptions = this.listOptions.actions.POST.sls.choices
      })
    },
    getList() {
